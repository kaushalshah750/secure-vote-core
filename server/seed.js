...
// --- HELPER: ROBUST PHONE CLEANER ---
function cleanPhoneNumber(raw) {
    if (!raw) return null;
    let p = raw.toString().replace(/[^0-9]/g, ''); // Digits only
...
const seedDB = async () => {
    try {
        ...
        await Config.create({ key: 'ELECTION_CONFIG', endTime, isActive: true });
        ...
        // **FIX: Set to track duplicates in this run**
        ...
        lines.forEach((line, index) => {
            ...
            if (parts.length >= 4) {
                ...
                if (!finalPhone) finalPhone = cleanPhoneNumber(rawMob2);

                if (finalPhone) {
                    // **FIX: Check if duplicate before adding**
                    if (seenNumbers.has(finalPhone)) {
                        rejectedLog.push(`Row ${index+1}: DUPLICATE PHONE - ${name} (${finalPhone})`);
                    } else {
                        seenNumbers.add(finalPhone); // Mark as seen
                        validVoters.push({
                            phone: finalPhone,
                            name: name,
                            hasVoted: false
                    ...
                    rejectedLog.push(`Row ${index+1}: INVALID PHONE - ${name}`);
                }
            }
        });
        ...
        if (validVoters.length > 0) {
            await Voter.bulkCreate(validVoters);
            console.log(`🎉 SUCCESS: Added ${validVoters.length} Voters.`);
        }

        // 5. LOGS
        if (rejectedLog.length > 0) {
            console.log('\n⚠️ REJECTED / DUPLICATE MEMBERS:');
            console.log(rejectedLog.join('\n'));
            console.log(`Total Rejected: ${rejectedLog.length}`);
        }
